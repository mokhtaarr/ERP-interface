...
@Component({
  selector: 'app-update-sales-invoice-details',
  templateUrl: './update-sales-invoice-details.component.html',
  styleUrls: ['./update-sales-invoice-details.component.scss']
})
export class UpdateSalesInvoiceDetailsComponent implements OnInit{
  ...
  constructor(
    ...
    private ref: MatDialogRef<UpdateSalesInvoiceDetailsComponent>,
  ...
  onUnitChange(event: MatSelectChange){
   ...
   onSumbit(){
    this.salesService.updateMsSalesInvoice(this.itemCollectionForm.value).subscribe(res=>{
      if(res){
        this.closepopup();
      }
    })
