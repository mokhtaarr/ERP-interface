...
@Component({
  selector: 'app-update-ms-return-sales-item-cards',
  templateUrl: './update-ms-return-sales-item-cards.component.html',
  styleUrls: ['./update-ms-return-sales-item-cards.component.scss']
})
export class UpdateMsReturnSalesItemCardsComponent implements OnInit{
  ...
  constructor(
    ...
    private ref: MatDialogRef<UpdateMsReturnSalesItemCardsComponent>,
  ...
  fillForm(){
    if(this.itemCollectionDataForm){
      this.itemCollectionForm.setValue({
        ...
        retItemCardId: this.itemCollectionDataForm.retItemCardId ?? null,
      });
  ...
  onUnitChange(event: MatSelectChange){
   ...
   onSumbit(){
    this.salesService.updateMsReturnSalesItemCards(this.itemCollectionForm.value).subscribe(res=>{
      if(res.status){
        this.closepopup();
      }
    })
