...
export class SalesOfferComponent  implements OnInit {
  ...
  ngOnInit(): void {
    this.salesOfferForm.disable();
    this.getAllSysBooks();
    this.GetAllCurrency();
    this.GetAllSysAnalyticalCodes();
    this.GetAllSalesOffer();
    this.getSalesEmployess();
    this.getAllCustomers();
  }

  GetAllSalesOffer(){
    this.salesService.getAllSalesOffer().subscribe(res=>{
      this.AllSalesOffer = res;
    })
  }
  ...
  getAllSysBooks(){
    this.salesService.getAllBooksForSalesOffer().subscribe(res=>{
      if(res.length > 0){
  ...
  getUserInfo(){
    this.accountService.currentUser$.subscribe(res =>{
      this.salesOfferForm.get('CreatedBy')?.setValue(res!.userId)
      this.salesOfferForm.get('storeId')?.setValue(res!.storeId)
      })
  }
  ...
  onSumbit() {
    ...
    if(this.invoiceItemsData != null){
      this.salesOfferForm.get("invoiceItem")?.setValue(this.invoiceItemsData)
    }

   this.salesService.AddMsSalesOffer(this.salesOfferForm.value).subscribe(res=>{
    if(res.status){
      this.GetAllSalesOffer();
      this.salesOfferForm.disable();
      this.salesOfferForm.get('salesOfferId')?.setValue(res.id);
  ...
  getLastRowData(){
    const LastItem = this.AllSalesOffer[this.AllSalesOffer.length - 1];
    if (LastItem.salesOfferId > 0) {
      this.salesOfferForm.setValue({
        salesOfferId: LastItem.salesOfferId,
      ...
      this.salesService.getMsSalesOfferItems(this.salesOfferForm.value.salesOfferId).subscribe(res=>{
  ...
  getNextRowData(){
    const index = this.AllSalesOffer.findIndex(
      (p) => p.salesOfferId == this.salesOfferForm.value.salesOfferId
    );

    const nextItem = this.AllSalesOffer[index + 1];

    if (nextItem.salesOfferId > 0) {
      this.salesOfferForm.setValue({
        salesOfferId: nextItem.salesOfferId,
      ...
      const LastItem = this.AllSalesOffer.findIndex(
        (p) => p.salesOfferId == this.salesOfferForm.value.salesOfferId
      );

      if (this.AllSalesOffer.length - 1 === LastItem) {
        this.DisabledNextButton = true;
        this.lastRow = true;
      }
  ...
  getPrevRowData(){
    ...
    const PrevItem = this.AllSalesOffer[index - 1];

    if (PrevItem.salesOfferId > 0) {
      this.salesOfferForm.setValue({
        salesOfferId: PrevItem.salesOfferId,
      ...
      const firstItem = this.AllSalesOffer.findIndex(
        (p) => p.salesOfferId == this.salesOfferForm.value.salesOfferId
      );
  ...
  getFirstRowData(){
    ...
    const FirstItem = this.AllSalesOffer[0];
    if (FirstItem.salesOfferId > 0) {
      this.salesOfferForm.setValue({
        salesOfferId: FirstItem.salesOfferId,
  ...
  undo(){
    ...
    if (this.undoIndex != -1) {
      const undoItem = this.AllSalesOffer[this.undoIndex];

      if (undoItem.salesOfferId > 0) {
        this.DeleteDisable = false;
        this.UpdateDisable = false;
        this.salesOfferForm.setValue({
          salesOfferId: undoItem.salesOfferId,
  ...
  updatePurchaseInvoice(){
    ...
    this.undoIndex = this.AllSalesOffer.findIndex(p=>p.salesOfferId == this.salesOfferForm.value.salesOfferId);
  ...
  New(){
    ...
    this.undoIndex = this.AllSalesOffer.findIndex(
      (p) => p.salesOfferId == this.salesOfferForm.value.salesOfferId
    );
    ...
    this.salesService.getCurrentTime().subscribe(res=>{
      this.salesOfferForm.get('trDate')?.setValue(res.currentDateTime);
      this.salesOfferForm.get('invDueDate')?.setValue(res.currentDateTime);
    })
    ...
    this.salesOfferForm.get('termId')?.setValue(this.AllTerms[0]?.termId);
    this.salesOfferForm.get('bookId')?.setValue(this.AllSysBooks[0]?.bookId);
  ...
  searchCustomer(searchValue: string): void {
    ...
    if (this.FilteredAllCustomers.length > 0) {
      this.salesOfferForm.get('customerId')?.setValue(this.FilteredAllCustomers[0].customerId);

      this.salesOfferForm.get('currencyId')?.setValue(this.FilteredAllCustomers[0].currencyId);
      const targetRate = this.AllCurrency.find(currency => currency.currencyId === this.FilteredAllCustomers[0].currencyId);
      this.salesOfferForm.get('rate')?.setValue(targetRate.rate);
  ...
  onAccountSelect(event: MatSelectChange): void {
    ...
    // this.salesOfferForm.get('currencyId')?.setValue(selectedVendor.currencyId);
  }
  ...
  updateItemCollectionFromDataBase(invoiceItem : any){
    var _popup = this.dialog.open(UpdateSalesOfferDetailsComponent, {
      width: '90%',
      enterAnimationDuration: '1000ms',
      exitAnimationDuration: '1000ms',
      data: {
  ...
  searchSalesEmployess(searchValue: string): void {
    ...
    if (this.FilteredAllSalesEmployess.length > 0) {
      this.salesOfferForm.get('empId')?.setValue(this.FilteredAllSalesEmployess[0].empId);
    }
  }
