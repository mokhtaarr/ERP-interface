...
@Component({
  selector: 'app-update-sales-offer-item-request',
  templateUrl: './update-sales-offer-item-request.component.html',
  styleUrls: ['./update-sales-offer-item-request.component.scss']
})
export class UpdateSalesOfferItemRequestComponent implements OnInit{
  ...
  constructor(
    ...
    private ref: MatDialogRef<UpdateSalesOfferItemRequestComponent>,
  ...
  fillForm(){
    if(this.itemCollectionDataForm){
      this.itemCollectionForm.setValue({
        ...
        salesOffertemCardReqId: this.itemCollectionDataForm.salesOffertemCardReqId ?? null,
      });
  ...
  onUnitChange(event: MatSelectChange){
   ...
   onSumbit(){
    this.salesService.updateMsSalesOfferRequest(this.itemCollectionForm.value).subscribe(res=>{
      if(res){
        this.closepopup();
      }
    })
