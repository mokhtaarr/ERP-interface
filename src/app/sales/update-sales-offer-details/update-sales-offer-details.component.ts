...
@Component({
  selector: 'app-update-sales-offer-details',
  templateUrl: './update-sales-offer-details.component.html',
  styleUrls: ['./update-sales-offer-details.component.scss']
})
export class UpdateSalesOfferDetailsComponent implements OnInit{
  ...
  constructor(
    ...
    private ref: MatDialogRef<UpdateSalesOfferDetailsComponent>,
  ...
  fillForm(){
    if(this.itemCollectionDataForm){
      this.itemCollectionForm.setValue({
        ...
        salesOffertemCardId: this.itemCollectionDataForm.salesOffertemCardId ?? null,
      });
  ...
  onUnitChange(event: MatSelectChange){
   ...
   onSumbit(){
    this.salesService.updateMsSalesOffer(this.itemCollectionForm.value).subscribe(res=>{
      if(res){
        this.closepopup();
      }
    })
